...
def generate_excel(records: List[Dict[str, Any]], output_path: str):
    ...
    # --- Crucial Step: Ensure all required columns exist ---
    # Add any missing columns from our template and fill with "Information not found"
    for col in ORDERED_HEADERS:
        if col not in df.columns:
            df[col] = NOT_FOUND

    # --- Enforce the exact column order and selection ---
    # This reorders the DataFrame to match the required template exactly.
